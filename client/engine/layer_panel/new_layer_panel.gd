...
func render() -> void:
	...
	if show_layer_type == "blocks":
		...
		target_layer = layers.get_target_map_layer()
	if show_layer_type == "art":
...
func _row_pressed(layer_name: String, x: float, y: float):
	if show_layer_type == "blocks":
		if layers.get_target_map_layer() == layer_name:
			var layer = layers.map_layers.get_node(layers.get_target_map_layer())
...
func _z_axis_change(new_z_axis: int):
	if show_layer_type == "blocks" and new_z_axis != layers.map_layers.get_node(layers.get_target_map_layer()).z_axis:
		emit_signal("level_event", {
...
func _rotation_change(new_rotation: int):
	if show_layer_type == "blocks" and new_rotation != layers.map_layers.get_node(layers.get_target_map_layer()).tile_map_rotation:
		emit_signal("level_event", {
...
func _set_layer_name(new_layer_name: String):
	if show_layer_type == "blocks" and layers.map_layers.get_node(layers.get_target_map_layer()).layer_name != new_layer_name:
		emit_signal("level_event", {
