...
func _set_tile(event: Dictionary, coords: Vector2i, coords_key: String, new_timestamp: int = -1) -> void:
	...
	var block_settings = {"settings": {}, "edited_settings": {}}
	if event.block_settings_info:
		block_settings = event.block_settings_info
	if event.block_id:
		tile_map_layer.add_block(coords, str(event.block_id), block_settings)
	else:
