...
@onready var extra_options_dropdown_button = $Buttons/ExtraOptionsButton/TextureButton
...
var editor_jump_menu_popup = preload("res://pages/editor/editor_jump_menu_popup.gd")
...
func _ready() -> void:
	...
	extra_options_button.pressed.connect(_on_extra_options_pressed)
...
func _on_extra_options_pressed():
	PopupManager.add_custom_popup(editor_jump_menu_popup, {"current_editor": current_editor, "popup_position": Vector2(extra_options_button.global_position.x - (240.0 - (extra_options_button.size.x / extra_options_button.scale.x)), extra_options_button.global_position.y + (extra_options_button.size.y / extra_options_button.scale.y))}, self)
