...
signal bounce_side_settings_changed
...
func _ready() -> void:
	...
	connect_node(self, "bounce_side_settings_changed")


func _change_bounciness(new_bounciness: float):
	bounciness = new_bounciness
	emit_signal("bounce_side_settings_changed", {"bounciness": bounciness, "speed_limit": speed_limit})
...
func set_side_settings(new_side_settings: Dictionary):
	if new_side_settings.has("bounciness"):
		bounciness = clamp(new_side_settings.bounciness, 0.0, 99999999.9)
		bounciness_box._update_text(str(bounciness))
	if new_side_settings.has("speed_limit"):
		speed_limit = clamp(new_side_settings.speed_limit, 0.0, 99999999.9)
		speed_limit_box._update_text(str(speed_limit))
	side_settings = {"bounciness": bounciness, "speed_limit": speed_limit}
