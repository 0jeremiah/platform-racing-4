...
@onready var show_block_picker_button = $ShowBlockPickerButton
...
var block_picker_rect = Rect2(Vector2(0.0, 0.0), Vector2(0.0, 0.0))
...
func _ready() -> void:
	...
	show_block_picker_button.pressed.connect(_show_block_picker)
...
func _process(delta: float) -> void:
	if check_if_not_clicking:
		...
		if !Input.is_mouse_button_pressed(MOUSE_BUTTON_LEFT):
			if quick_click:
				if (from_block_picker and block_picker_rect.has_point(get_global_mouse_position())) or (blocks_container.get_local_mouse_position() != old_mouse_position and Rect2(Vector2.ZERO, scroll_container.size).has_point(scroll_container.get_local_mouse_position())):
...
func _show_block_picker():
	PopupManager.add_custom_popup(blockpicker_popup, {"blockpicker_clicked_func": Callable(self, "_drag_block"), "close_after_select": false, "popup_position": Vector2(show_block_picker_button.global_position.x, show_block_picker_button.global_position.y + (show_block_picker_button.size.y + 20)), "send_block_picker_size_and_position_func": Callable(self, "set_block_picker_rect")}, self)
...
func set_block_picker_rect(block_picker_info: Dictionary):
	block_picker_rect = Rect2(block_picker_info.position, block_picker_info.size)
