...
func get_teleport_distance(horizontal_distance: float, vertical_distance: float) -> Vector2:
	return Vector2(Settings.tile_size.x * horizontal_distance, Settings.tile_size.y * vertical_distance)
...
func set_visuals(_character: Character):
	...
	visual_aid.global_position = Vector2(_character.position.x + (teleport_distance.x * _character.movement.facing), _character.position.y + teleport_distance.y)
...
func _teleport(_character):
	...
	var teleport_distance = get_teleport_distance(GameConfig.get_value("items-effects", "teleport_horizontal_distance"), GameConfig.get_value("items-effects", "teleport_vertical_distance"))
	_character.position += Vector2(teleport_distance.x * _character.movement.facing, teleport_distance.y)
