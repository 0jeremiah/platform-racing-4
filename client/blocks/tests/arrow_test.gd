extends GutTest
## Test for arrow block behavior
## Expects: Ball lands on arrow block, gets pushed by arrow block


const ARROW_COORDS := Vector2i(4, 4)
...
func _setup_visual_test() -> void:
	...
	var arrow_config := BlockTestUtils.load_config("res://blocks/configs/arrow_up.json")
	var tile_map_layer: ConfigurableTileMapLayer = $TileMapLayer
	tile_map_layer.setup_from_configs([arrow_config])
	tile_map_layer.set_cell_by_id(ARROW_COORDS, arrow_config.id)

	# Setup ball
	var ball: RigidBody2D = $Ball
	var mine_world_pos: Vector2 = Vector2(ARROW_COORDS * Settings.tile_size) + Vector2(Settings.tile_size_half)
	...
	var mine_exists := tile_map_layer.get_cell_source_id(ARROW_COORDS) != -1
...
func _create_test_scene() -> Node2D:
	...
	var scene_template: PackedScene = load("res://blocks/tests/arrow_test.tscn")
